...
(function (window) {
  ...
  // 沿用 v3 存储 key（version 字段区分，保证旧数据不丢失）
  LS.STORAGE_KEY = 'life-system-v3';
  ...
  function defaultState() {
    return {
      ...
      tasks: [],
      assets: { balance: 0, monthlyGoal: 500, transactions: [] },
      streak: 0,
      lastActiveDate: null,
  ...
  LS.loadState = function () {
    ...
    try {
      const saved = localStorage.getItem(LS.STORAGE_KEY);
      if (saved) {

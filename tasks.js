...
(function (window) {
  ...
  LS.renderTasks = function () {
    ...
    container.innerHTML = buildScheduleCard() + buildScheduleToolbar() + buildTaskTreeHtml();
  };
  ...
  function buildScheduleCard() {
    ...
    const openTasks = LS.state.tasks.filter(t => !t.completed && t.status !== 'abandoned');
    const todayDone = ((LS.state.taskHistory.find(h => h.date === today) || {}).count || 0);
    let daysDone = 0;
    for (let i = 0; i < 30; i++) {
      const d = new Date(); d.setDate(d.getDate() - i);
      const k = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
      if (((LS.state.taskHistory.find(h => h.date === k) || {}).count || 0) > 0) daysDone++;
    }
    const total = openTasks.length;
    const pct = Math.round(daysDone / 30 * 100);
    const donePct = total > 0 ? Math.round(todayDone / total * 100) : 0;
    return '<div class="schedule-card">' +
      '<div class="schedule-ring" style="--p:' + donePct + '"><div class="schedule-ring-num">' + todayDone + '</div>' +
      '<div class="schedule-ring-label">/' + total + '</div></div>' +
      '<div class="schedule-card-info"><div class="schedule-card-title">今日完成</div>' +
      '<div class="schedule-card-desc">' + (total ? '还有 ' + (total - todayDone) + ' 件事，稳步推进' : '今日无待办，享受自由时间 ☀️') + '</div>' +
      '<div class="schedule-30d">📅 过去 30 天完成 ' + daysDone + ' 天（' + pct + '%）</div></div></div>';
  }

  function buildScheduleToolbar() {
    return '<div class="tasks-topbar" style="justify-content:space-between;margin-bottom:var(--sp-md)">' +
      '<button class="btn btn-primary btn-sm" onclick="LS.openAddTaskModal()">＋ 新建任务</button>' +
      '<div style="display:flex;gap:var(--sp-xs)">' +
      '<button class="btn-ghost btn-sm log-btn" onclick="LS.openFocusModal()">🍅 专注</button>' +
      '<button class="btn-ghost btn-sm log-btn" onclick="LS.openCompletionLog()">📜 记录</button></div></div>';
  }
  ...
  function buildTaskTreeHtml() {
    ...
    TASK_GROUP_ORDER.forEach(cat => {
      ...
      html += '<div class="task-group' + (collapsed ? ' collapsed' : '') + '">' +
        ...
        '<span class="task-group-count">' + doneCount + '/' + tasks.length + '</span>' +
        '</button>' +
        '<div class="task-group-body"' + (collapsed ? ' style="display:none"' : '') + '">' +
        sorted.map(buildScheduleItem).join('') +
        '</div></div>';
    });

    if (!hasAny) {
      html += '<div class="empty-state"><div class="empty-state-icon">🗓️</div>' +
        '<div class="empty-state-text">暂无日程<br>点「＋ 新建任务」添加第一条</div></div>';
    }
    html += '</div>';
    return html;
  }
  ...
  // 精简日程项（勾选即完成）
  // ============================================
  function buildScheduleItem(task) {
    const done = !!task.completed;
    const isOverdue = !done && LS.isTaskOverdue(task);
    const cls = ['task-item', 'schedule-item', done ? 'completed' : '', isOverdue ? 'overdue' : ''].filter(Boolean).join(' ');
    const attr = LS.state.attributes.find(a => a.id === task.attributeId);
    const skill = LS.state.skills.find(s => s.id === task.skillId);
    let reward = '';
    if (attr && task.points > 0) reward = '<span class="schedule-reward">' + LS.escapeHtml(attr.name) + ' +' + task.points + '</span>';
    else if (skill && task.xp > 0) reward = '<span class="schedule-reward">' + LS.escapeHtml(skill.name) + ' +' + task.xp + '</span>';
    else if (task.customReward) reward = '<span class="schedule-reward">🎁 ' + LS.escapeHtml(task.customReward) + '</span>';
    let due = '';
    if (done) due = '<span class="task-due done">✓ 已完成</span>';
    else if (task.timeType === 'unlimited') due = '<span class="task-due unlimited">每日</span>';
    else { const d = LS.getTaskDueDay(task); if (d) due = '<span class="task-due' + (isOverdue ? ' overdue' : '') + '">' + d + (isOverdue ? ' · 逾期' : '') + '</span>'; }
    if (task.repeat) due += '<span class="schedule-repeat">🔁</span>';
    return '<div class="' + cls + '">' +
      '<button class="schedule-check' + (done ? ' on' : '') + '" onclick="LS.completeTask(\'' + task.id + '\')" title="' + (done ? '已完成' : '标记完成') + '">' + (done ? '✓' : '') + '</button>' +
      '<div class="schedule-main"><div class="task-title">' + LS.escapeHtml(task.title) + '</div>' +
      ((reward || due) ? '<div class="schedule-sub">' + reward + ' ' + due + '</div>' : '') + '</div>' +
      '<div class="task-actions">' +
      (done ? '' : '<button class="btn-icon" onclick="LS.openCompleteTaskModal(\'' + task.id + '\')" title="完成并备注"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg></button>') +
      '<button class="btn-icon danger" onclick="LS.deleteTask(\'' + task.id + '\')" title="删除"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2m3 0v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg></button>' +
      '</div></div>';
  }

...
(function (window) {
  ...
  LS.generateLicenseKey = function (type) {
    ...
    const check = ALPHABET[mod36(hash(payload + SECRET + (type || 'lifetime')))];
  ...
  LS.validateLicenseKey = function (key) {
    ...
    let plan = null;
    if (check === ALPHABET[mod36(hash(payload + SECRET + 'lifetime'))]) plan = 'lifetime';
    else if (check === ALPHABET[mod36(hash(payload + SECRET + 'monthly'))]) plan = 'monthly';

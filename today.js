...
(function (window) {
  ...
  LS.renderToday = function () {
    ...
    // 系统建议（智能提示）
    const tips = [];
    const focusMins = LS.getTodayFocus();
    if (focusMins > 0) tips.push('🍅 今日已专注 ' + focusMins + ' 分钟，继续保持');
    const decaying = LS.state.attributes.filter(a => LS.isDecaying(a));
    if (decaying.length) tips.push('⚠️ 属性「' + decaying.map(a => a.name).join('、') + '」正在衰减，快完成关联任务');
    if (dueToday.length && !doneToday) tips.push('💡 今天还有 ' + dueToday.length + ' 个任务待完成');
    if (LS.isPro && !LS.isPro()) tips.push('👑 升级 PRO 解锁方案库、月报与快照备份');
    if (tips.length) {
      html += '<div class="today-tips">' + tips.slice(0, 2).join('<br>') + '</div>';
    }
    html += '</div>';
    slot.innerHTML = html;
  };
